    ...
    <StyledRoot>
      <Container sx={{ height: 'inherit', minHeight: 'inherit' }} component={MotionViewport}>
        ...
        <Stack
          ...
          <Stack direction={isMobile ? 'column-reverse' : 'row'} minHeight='300px' maxHeight='50vh' spacing={'1rem'}>
            <Stack flex={1} pt={2} alignItems='center' justifyContent={'space-evenly'} maxHeight='200px'>
            ...
            <Image src="/assets/images/venue.png" alt="venue" flex={1} height='100%'
              sx={{ width: isMobile ? { xs: '300px', sm: '360px' } : '560px', alignSelf: 'center' }}
            />
          </Stack>

          <Box sx={{ position: 'relative', borderRadius: '50px', alignItems: 'center', justifyContent: 'center', alignContent: 'center', justifyItems: 'center', overflow: 'hidden', height: isMobile ? '300px' : '500px', maxHeight: isMobile ? 'F' : '50vh', pointerEvents: 'auto' }}>
            <Box sx={{ pointerEvents: 'none', position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, boxShadow: 'inset 0px 4px 4px rgba(0, 0, 0, 0.25)', zIndex: 1 }} />
            <iframe
              title='map'
